...
- **Supersampling is now available in Escha & Logy and Shallie.** Those games already size every full-frame target from their own settings file, so the launcher writes it at base times factor and the mod clamps the swap chain back to the base, resolves the larger frame into it, and corrects the composite's viewport. The whole frame is supersampled rather than the scene alone, interface included. Confirmed in game in both titles.
- **Edge smoothing is now available in Escha & Logy and Shallie.** It runs over the finished frame there rather than before the interface is composited, because no scene-target rule exists for that renderer yet, so menu text is softened along with the scene. That is why it is off unless asked for, where Ayesha has it on by default. Not yet validated in game on either title.
