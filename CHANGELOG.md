...
- **The travel-map cursor in Ayesha and in Escha & Logy.** It moved a fixed distance per frame rather than per second, crossing the map more than three times too fast at 200 Hz. At 60 fps and below nothing changes. Shallie needs no correction: the same code there already accounts for frame time, which is how the defect in the other two was confirmed rather than guessed at.
...
- **Edge smoothing (SMAA)**, in all three games, and in all three it runs before the interface is drawn, so menu text stays sharp. On Escha & Logy and Shallie the moment it runs at was found by recording one frame's structure: the scene lands in a surface in a single draw, and the interface is drawn into that same surface immediately afterwards. Antialiasing in between reaches everything the player sees and nothing they read.
- **Sharpening**, in all three games. Runs on the same surface straight after edge smoothing, which softens a little as it smooths, and works on its own if edge smoothing is off. It adapts to local contrast, so flat areas and already-hard edges are left alone. Off, low, medium and high.
