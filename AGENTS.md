...
- `src/engines/ktgl/` — Escha & Logy and Shallie (LTGL/KTGL, UCRT). Fingerprinting and the engine-specific address fixes, including loading text, startup flow, save integrity, world-map movement and Shallie's control prompt. Each detour or patch is gated by the recognized executable row and its own expected-byte window.
