...
MSAA joins it as the other antialiasing option, and both are set from the launcher. An earlier version of this note claimed Ayesha already multisamples its 3D scene at 4x and that the setting merely raises or lowers that; draw-time instrumentation disproved it. The engine allocates six multisampled targets at startup and renders into none of them, so the mod builds MSAA outside the renderer with multisample twin resources. There is no "game default" to fall back to — off is off.
