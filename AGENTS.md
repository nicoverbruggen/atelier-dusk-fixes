...
`scripts/check_default_ini.py` enforces this and runs in CI: it checks that every option the code reads is documented, that nothing documented is unread, that the literal defaults agree, and that the launcher's own fallbacks agree too — the launcher keeps a separate copy of every default, and one that drifts does not just display the wrong value, it writes it back on the next Save. An option deliberately kept out of `default.ini` goes in the allowlist at the top of that script rather than being dropped from the check. One is there now: `AnisotropicFiltering` is on by default only on Ayesha, so no single shipped value can be correct for all three games. `SMAA` was there for the same reason until it became on by default everywhere, at which point it gained a `default.ini` line and left the allowlist.
