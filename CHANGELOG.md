...
- **Sharpening**, in all three games. Runs on the same surface straight after edge smoothing, which softens a little as it smooths, and works on its own if edge smoothing is off. It adapts to local contrast, so flat areas and already-hard edges are left alone. Off, light, medium and strong.
